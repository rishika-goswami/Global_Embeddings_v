...
def apply_jm_smoothing(local_label, global_priors, jm_lambda):
    """
    Applies Jelinek-Mercer Smoothing (Linear Interpolation).
    P_smooth = (1 - lambda) * Local_Prob + lambda * Global_Prior
    
    Returns the winning label (argmax) and its smoothed confidence score.
    """
    smoothed_probs = {}
    
    for label, prior in global_priors.items():
        # Local prob is 1.0 if it matches the LLM's choice, else 0.0
        local_prob = 1.0 if label == local_label else 0.0
        
        # Jelinek-Mercer linear interpolation
        smoothed_prob = (1.0 - jm_lambda) * local_prob + jm_lambda * prior
...
# 4. Graph Generation
def generate_graphs(df, jm_lambda, output_dir):
    sns.set_theme(style="whitegrid", palette="muted")
    lam_str = str(jm_lambda)
    
    # 1. Aspect Distribution
    plt.figure(figsize=(10, 6))
    sns.countplot(data=df, y="Aspect", order=df['Aspect'].value_counts().index, color="#2b2d42")
    plt.title(f"Holdout Aspects (JM Smoothed, λ={lam_str})", fontsize=14, fontweight='bold')
    plt.xlabel("Count")
    plt.ylabel("Aspect Category")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"jm_{lam_str}_aspect_distribution.png"), dpi=300)
    plt.close()
    ...
    plt.title(f"Holdout Frustration vs Request (JM Smoothed, λ={lam_str})", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"jm_{lam_str}_signal_matrix.png"), dpi=300)
    plt.close()
    ...
    plt.title(f"Holdout Priority Levels (JM Smoothed, λ={lam_str})", fontsize=14, fontweight='bold')
    plt.ylabel("Number of Complaints")
    plt.xticks(rotation=15)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"jm_{lam_str}_priority_levels.png"), dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    # 3. Set the Jelinek-Mercer smoothing parameter (lambda)
    JM_LAMBDA = 0.8 
    print(f"\nApplying Jelinek-Mercer Smoothing to Holdout Set with λ = {JM_LAMBDA}")
    
    # Setup Dynamic Output Directory
    output_dir = os.path.join("results", "jelinek-mercer", str(JM_LAMBDA))
    os.makedirs(output_dir, exist_ok=True)
    
    # 4. Apply smoothing and extract both the label and the calibrated confidence
    df_holdout[['Aspect', 'Aspect_Conf']] = df_holdout['Raw_Aspect'].apply(
        lambda x: pd.Series(apply_jm_smoothing(x, aspect_priors, JM_LAMBDA))
    )
    df_holdout[['Frustration', 'Frust_Conf']] = df_holdout['Raw_Frustration'].apply(
        lambda x: pd.Series(apply_jm_smoothing(x, frust_priors, JM_LAMBDA))
    )
    df_holdout[['Request', 'Req_Conf']] = df_holdout['Raw_Request'].apply(
        lambda x: pd.Series(apply_jm_smoothing(x, req_priors, JM_LAMBDA))
    )

    # 5. Re-evaluate Priority based on smoothed labels
    df_holdout['Priority'] = df_holdout.apply(lambda row: assign_priority(row['Frustration'], row['Request']), axis=1)

    # 6. Save CSV and Graphs
    csv_path = os.path.join(output_dir, f"jm_{JM_LAMBDA}_holdout_smoothed_results.csv")
    df_holdout.to_csv(csv_path, index=False)
    print(f"Saved smoothed holdout data (with confidence scores) to {csv_path}")
    
    generate_graphs(df_holdout, JM_LAMBDA, output_dir)
...
def get_unique_classes(df, column):
    """Extracts the unique vocabulary (V) for a given categorical column."""
    return df[column].unique().tolist()

def apply_bayesian_smoothing(local_label, classes, b_constant):
    """
    Applies Additive Bayesian Smoothing (Uniform Prior).
    P_smooth = (Local_Count + b) / (1 + b * V)
    
    Returns the smoothed probabilities to demonstrate confidence calibration,
    and the final winning label (argmax).
    """
    smoothed_probs = {}
    V = len(classes)
    ...
    for label in classes:
        ...
        # Bayesian Additive calculation
        smoothed_prob = (local_count + b_constant) / (1.0 + b_constant * V)
        smoothed_probs[label] = smoothed_prob
        
    # Return the label with the highest smoothed probability
    winning_label = max(smoothed_probs, key=smoothed_probs.get)
    return winning_label, smoothed_probs[winning_label]

def assign_priority(frustration, request):
    """Re-calculates priority based on signals."""
...
# 3. LLM Functions for the Holdout Set (Shared Cache Logic)
...
# 4. Graph Generation
def generate_graphs(df, b_constant, output_dir):
    sns.set_theme(style="whitegrid", palette="muted")
    b_str = str(b_constant)
    
    # 1. Aspect Distribution
    plt.figure(figsize=(10, 6))
    sns.countplot(data=df, y="Aspect", order=df['Aspect'].value_counts().index, color="#2b2d42")
    plt.title(f"Holdout Aspects (Bayesian Smoothed, b={b_str})", fontsize=14, fontweight='bold')
    plt.xlabel("Count")
    plt.ylabel("Aspect Category")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"b_{b_str}_aspect_distribution.png"), dpi=300)
    plt.close()
    ...
    plt.title(f"Holdout Frustration vs Request (Bayesian Smoothed, b={b_str})", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"b_{b_str}_signal_matrix.png"), dpi=300)
    plt.close()
    ...
    plt.title(f"Holdout Priority Levels (Bayesian Smoothed, b={b_str})", fontsize=14, fontweight='bold')
    plt.ylabel("Number of Complaints")
    plt.xticks(rotation=15)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"b_{b_str}_priority_levels.png"), dpi=300)
    plt.close()
...
if __name__ == "__main__":
    # 1. Load the Baseline data to extract the full Vocabulary (V)
    ...
    print("Extracting full Vocabulary (V) from the 70% baseline set...")
    aspect_classes = get_unique_classes(df_baseline, 'Aspect')
    frust_classes = get_unique_classes(df_baseline, 'Frustration')
    req_classes = get_unique_classes(df_baseline, 'Request')
    ...
    # 3. Set the Bayesian smoothing constant (b)
    B_CONSTANT = 1.0 
    print(f"\nApplying Bayesian Additive Smoothing to Holdout Set with b = {B_CONSTANT}")
    
    output_dir = os.path.join("results", "bayesian", str(B_CONSTANT))
    os.makedirs(output_dir, exist_ok=True)
    
    # 4. Apply smoothing using the vocabulary against the raw holdout labels
    df_holdout[['Aspect', 'Aspect_Conf']] = df_holdout['Raw_Aspect'].apply(
        lambda x: pd.Series(apply_bayesian_smoothing(x, aspect_classes, B_CONSTANT))
    )
    df_holdout[['Frustration', 'Frust_Conf']] = df_holdout['Raw_Frustration'].apply(
        lambda x: pd.Series(apply_bayesian_smoothing(x, frust_classes, B_CONSTANT))
    )
    df_holdout[['Request', 'Req_Conf']] = df_holdout['Raw_Request'].apply(
        lambda x: pd.Series(apply_bayesian_smoothing(x, req_classes, B_CONSTANT))
    )

    # 5. Re-evaluate Priority
    df_holdout['Priority'] = df_holdout.apply(lambda row: assign_priority(row['Frustration'], row['Request']), axis=1)

    # 6. Save CSV and Graphs
    csv_path = os.path.join(output_dir, f"b_{B_CONSTANT}_holdout_smoothed_results.csv")
    df_holdout.to_csv(csv_path, index=False)
    print(f"Saved smoothed holdout data (with confidence scores) to {csv_path}")
    
    generate_graphs(df_holdout, B_CONSTANT, output_dir)